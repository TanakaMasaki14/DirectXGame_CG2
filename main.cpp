...
//デバックレイヤーをオンに
	...
	while (true) {
		...
		// 値を書き込むと自動的に転送される
		constMapMaterial->color = XMFLOAT4(Red, Green, 0, 0.5f);
		//DirectX毎フレーム処理　ここから
		// キーボード情報の取得開始
		keyboard->Acquire();
		...
		viewport[2].Width = WIN_WIDTH / 4;
		viewport[2].Height = WIN_HEIGHT / 4;
		...
		viewport[3].Height = WIN_HEIGHT / 4;
